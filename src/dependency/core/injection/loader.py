...
class InjectionLoader:
    ...
    def __init__(self, container: Container, providers: list[ProviderInjection]) -> None:
        ...
        self.resolved_layers: list[list[ProviderInjection]] = []
        super().__init__()
    ...
    def resolve_dependencies(self) -> None:
        ...
        while unresolved_providers:
            new_layer = [
                ...
                if provider_is_resolved(provider.dependency, self.resolved_layers)
            ]

            if len(new_layer) == 0:
                raise_providers_error(unresolved_providers, self.resolved_layers)
            self.resolved_layers.append(new_layer)
        ...
        named_layers = pformat(self.resolved_layers)
        ...
        unresolved_depends = [
            ...
            if not provider_is_resolved(depends, self.resolved_layers)]
        if unresolved_depends:
            raise_dependency_error(unresolved_depends, self.resolved_layers)

        for provider in self.providers:
            provider.do_prewiring()

        for resolved_layer in self.resolved_layers:
