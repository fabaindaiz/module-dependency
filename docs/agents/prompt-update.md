...
released:  2026-09-17
upstream:  m-7c41a9/main        # where this copy pulls updates from; "" = it is a root
adopted:   2026-09-17           # bootstrapped from version 0 of this lineage, commit 4ca02c7
# Local renamings and substitutions. The update does not re-propose any of these.
adapted:
  - The gate is `hatch run build:gate`. There is no generic verify command and none is added.
  - The structural audit script is `tools/audit_dependency.py`, with failures and advisories
    as the method describes.
  - The skills are `workflow`, `verify`, `release`, `state-review` and
    `troubleshoot-resolution`. The method's generic skill names are not used.
  - The session loop lives in `.claude/skills/workflow/SKILL.md`, not in a document of its
    own. Extending the skill that already described a change was cheaper than a fourth map.
  - The method set is excluded from the published docs via `exclude_docs: agents/` in
    `mkdocs.yaml`. It is instruction, not documentation for users of the library.
# Deltas deliberately not taken, with the reason. Not re-litigated unless the reason changes.
declined:
  - "v3 Workspaces: one repository, no monorepo and no second checkout. Nothing to coordinate."
  - "v3 Models, reasoning levels and cost: it governs how the method is run, not what this
    repository guarantees. In CLAUDE.md it would spend budget on something that constrains
    no change."
  - "v6 The pre-flight, applied to this repo's own skills: `verify`, `release` and
    `workflow` are short and reversible, and none of them burns a long run on a wrong
    assumption. Kept for the method's own executable prompts, where the cost is real."
```
